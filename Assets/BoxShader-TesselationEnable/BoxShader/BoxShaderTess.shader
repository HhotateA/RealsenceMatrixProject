...
Shader "HOTATE/BoxShader/BoxShaderTess"
{
	Properties
	{
		...
		[Space(80)]
		[Enum(Auto,0,UserSetting,1)] _Tesselationmode ("TesselationMode", float) = 0.0
		_autoTess ("AutoMaticTessellation",Range(1,100)) = 1
		_autoTessPow ("AutoTessellationPow",float) = 1
		_TessFactor ("Tess Factor",Vector) = (3,3,3,2)
	}
	SubShader
	{
		...
		Pass
		{
			...
			h2d_const HSConst(InputPatch<v2h,INPUT_PATCH_SIZE> i)
			{
				h2d_const o ;
				float4 outmaticTes;
				outmaticTes.x = (max(i[0].vertex.x,max(i[1].vertex.x,i[2].vertex.x))-min(i[0].vertex.x,min(i[1].vertex.x,i[2].vertex.x)))*_autoTess*_autoTess;
				outmaticTes.x = min(outmaticTes.x,pow(_autoTess,_autoTessPow));
				outmaticTes.y = (max(i[0].vertex.y,max(i[1].vertex.y,i[2].vertex.y))-min(i[0].vertex.y,min(i[1].vertex.y,i[2].vertex.y)))*_autoTess*_autoTess;
				outmaticTes.y = min(outmaticTes.y,pow(_autoTess,_autoTessPow));
				outmaticTes.z = (max(i[0].vertex.z,max(i[1].vertex.z,i[2].vertex.z))-min(i[0].vertex.z,min(i[1].vertex.z,i[2].vertex.z)))*_autoTess*_autoTess;
				outmaticTes.z = min(outmaticTes.z,pow(_autoTess,_autoTessPow));
				outmaticTes.w = max(outmaticTes.x,max(outmaticTes.y,outmaticTes.z));
				outmaticTes.w = min(outmaticTes.w,_autoTess);
				o.tess_factor[0] = lerp(min(_TessFactor.x,max(outmaticTes.x,1)),_TessFactor.x,_Tesselationmode);
				o.tess_factor[1] = lerp(min(_TessFactor.y,max(outmaticTes.y,1)),_TessFactor.y,_Tesselationmode);
				o.tess_factor[2] = lerp(min(_TessFactor.z,max(outmaticTes.z,1)),_TessFactor.z,_Tesselationmode);
				o.InsideTessFactor = lerp(min(_TessFactor.w,max(outmaticTes.w,1)),_TessFactor.w,_Tesselationmode);
				return o;
			}
